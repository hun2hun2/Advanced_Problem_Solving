...
def monopoly(numSides, numIterations):
    ...
    total = numSides * numSides
    dice = [(i+j, 1.0) for i in range(1, numSides+1) for j in range(1, numSides+1)]
    dice_probs = {}
    for s,_ in dice:
        dice_probs[s] = dice_probs.get(s, 0) + 1
    dice = [(s, cnt/total) for s, cnt in dice_probs.items()]
    ...
    for i in range(N):
        lst = []
        for dsum, p_d in dice:
            pos = (i + dsum) % N
            if pos == G2J:
                lst.append((JAIL, p_d))
            elif pos in CC:
                lst.append((pos, p_d * 14/16))
                lst.append((GO,  p_d * 1/16))
                lst.append((JAIL,p_d * 1/16))
    ...
    for _ in range(numIterations):
        pNext = [0.0]*N
        for i, pi in enumerate(pCurrent):
            if pi:
                for j, pij in transitions[i]:
                    pNext[j] += pi * pij
        pCurrent = pNext
    
    return [(squares[i], pCurrent[i]) for i in range(len(pCurrent))]
...
if __name__ == "__main__":
    ...
    if not correct: print("fail (since the algorithm is not correct)")
        ...
        print(f"Average running times of the submitted code {tSubmittedCode:.10f} and simpleGame*1.2 {tSpeedCompare*1.2:.10f}")
        if tSubmittedCode < tSpeedCompare * 1.2: print("pass")
        else: print("fail")        
        print()
        
        print(f"Average running times of the submitted code {tSubmittedCode:.10f} and simpleGame*0.6 {tSpeedCompare*0.6:.10f}")
        if tSubmittedCode < tSpeedCompare * 0.6: print("pass")
        else: print("fail")        
        print()

        print(f"Average running times of the submitted code {tSubmittedCode:.10f} and simpleGame*0.2 {tSpeedCompare*0.2:.10f}")
        if tSubmittedCode < tSpeedCompare * 0.2: print("pass")
        else: print("fail")        
        print()
